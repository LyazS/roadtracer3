import sys
sys.path.append('./lib/discoverlib')
import geom
import grid_index
...
class Vertex(object):
    ...
    def __repr__(self):
        return 'Vertex({}, {}, {} in {} out)'.format(self.id, self.point,
                                                     len(self.in_edges),
                                                     len(self.out_edges))
...
class EdgePos(object):
    ...
    def point(self):
        ...
        if vector.magnitude() < 1:
            ...
            return segment.start.add(
                vector.scale(self.distance / vector.magnitude()))

    def reverse(self):
        return EdgePos(self.edge.get_opposite_edge(),
                       self.edge.segment().length() - self.distance)
...
class Index(object):
    ...
    def search(self, rect):
        edge_ids = self.index.intersection(
...
class Graph(object):
    ...
    def edgeIndex(self):
        ...
        for edge in tqdm(self.edges,total=len(self.edges),leave=False):
            bounds = edge.bounds()
            rt.insert(
                edge.id,
    ...
    def clone(self):
        ...
        for edge in self.edges:
            e = other.add_edge(other.vertices[edge.src.id],
                               other.vertices[edge.dst.id])
        return other
        ...
        for other, edge in closestNode._neighbors().items():
            ...
            if other in remaining:
                if hasattr(edge, 'cost'):
                    ...
                    d = closestDistance + closestNode.point.distance(
                        other.point)
...
def closest_reachable_edge(point,
                           index,
                           explored_node_pairs=None,
                           remaining=50,
                           src=None,
    ...
    if src is None:
        candidates = [(edge, None) for edge in index.search(
            point.bounds().add_tol(distance_threshold))]
    else:
        ...
        def search_vertex(path, vertex, remaining):
            for edge in vertex.out_edges:
                if (edge.src.id, edge.dst.id) in cur_explored_node_pairs or (
                        edge.dst.id, edge.src.id) in cur_explored_node_pairs:
                    continue
        ...
        l = reverse_edge_pos.edge.segment().length(
        ) - reverse_edge_pos.distance
        if remaining > l:
            search_vertex([], reverse_edge_pos.edge.dst, remaining - l)

    for edge, path in candidates:
        if explored_node_pairs is not None and (
            (edge.src.id, edge.dst.id) in explored_node_pairs or
            (edge.dst.id, edge.src.id)
                in explored_node_pairs) and (src is None or edge != src.edge):
            continue
...
def follow_graph(edge_pos, distance, explored_node_pairs=None):
    ...
    def search_vertex(vertex, remaining):
        for edge in vertex.out_edges:
            if (edge.src.id, edge.dst.id) in explored_node_pairs or (
                    edge.dst.id, edge.src.id) in explored_node_pairs:
                continue
...
class RoadSegment(object):
    ...
    def is_opposite(self, rs):
        return self.src() == rs.dst() and self.dst() == rs.src(
        ) and self.edges[0].is_opposite(rs.edges[-1])
...
def get_graph_road_segments(g):
    ...
    def search_from_edge(rs, edge, direction):
        ...
        while True:
            ...
            edges = [
                next_edge for next_edge in edges
                if not next_edge.is_opposite(cur_edge)
            ]
...
def mapmatch(index, road_segments, edge_to_rs, points, segment_length):
    ...
    for edge in index.search(points[0].bounds().add_tol(START_TOL)):
        ...
        if rs.id not in probs or p > probs[rs.id][0]:
            rs_distance = rs.edge_distances[
                edge.id] + edge.segment().project_factor(points[0])
    ...
    def distance_to_rs(rs, clip_distance, point, vector):
        ...
        for edge in edges:
            rs_distance = rs.edge_distances[
                edge.id] + edge.segment().project_factor(point)
            rs_distance = numpy.clip(rs_distance, low_distance, high_distance)
            rs_distance = numpy.clip(
                rs_distance, rs.edge_distances[edge.id],
                rs.edge_distances[edge.id] + edge.segment().length())
            edge_distance = rs_distance - rs.edge_distances[edge.id]
            rs_point = edge.segment().point_at_factor(edge_distance)
            distance = rs_point.distance(
                point) + edge.segment().vector().angle_to(vector) * 12
    ...
    for i in range(len(points) - 1):
        ...
        for prev_rs_id in probs:
            ...
            rs_candidates = [(rs, prev_rs_distance)
                             ] + [(next_rs, -remaining_rs_distance)
                                  for next_rs in rs.out_rs(edge_to_rs)]
            for next_rs, clip_distance in rs_candidates:
                if next_rs.is_opposite(rs):
                    continue
                distance, rs_distance = distance_to_rs(
                    next_rs, clip_distance, points[i + 1],
                ...
                #if next_rs != rs:
                #	p += math.log(0.1)
                if next_rs.id not in next_probs or p > next_probs[
                        next_rs.id][0]:
...
def get_nearby_vertices_by_distance(vertex, distance):
    ...
    while len(search_queue) > 0:
        ...
        for edge in vertex.out_edges:
            ...
            search_queue.append(
                (edge.dst, remaining - edge.segment().length()))
    return nearby_vertices
