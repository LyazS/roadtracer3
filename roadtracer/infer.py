...
def fix_outputs(batch_angle_outputs, batch_stop_outputs):
    if batch_angle_outputs.shape[1] == 64:
        ...
        fixed_stop_outputs = numpy.zeros((batch_angle_outputs.shape[0], 2),
                                         dtype='float32')
        ...
        raise Exception("bad angle_outputs length={}".format(
            len(angle_outputs)))


def score_accuracy(stop_targets,
                   angle_targets,
                   stop_outputs,
                   angle_outputs,
                   threshold,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'outname', help='output filename to save inferred road network graph')
    parser.add_argument('--s',
                        help='stop threshold (default 0.4)',
                        default=0.4)
    parser.add_argument('--r',
                        help='region (default chicago)',
                        default='chicago')
    ...
    parser.add_argument('--e',
                        help='existing graph to get starting locations from')
    parser.add_argument(
        '--f',
        help=
        'filter threshold to filter output edges (e.g. 0.75, default disabled)',
        default=0)
    ...
    if args.j:
        ...
        tileloader.startlocs_path = os.path.join(args.j,
                                                 'starting_locations.json')
    ...
    if EXISTING_GRAPH_FNAME is None:
        ...
        if USE_TL_LOCATIONS:
            ...
            def match_point(p):
                best_pos = None
                best_distance = None
                for candidate in tile_data['gc'].edge_index.search(
                        p.bounds().add_tol(32)):
    ...
    if args.e:
        ...
        for edge in ng.edges:
            ...
            nearby_edges = path.edge_rtree.intersection(
