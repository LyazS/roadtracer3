...
TRAINING_REGIONS = [
    'indianapolis',
    # 'louisville', 'columbus', 'milwaukee', 'minneapolis',
...
def load_tile(region, i, j, mode='all'):
    ...
    sat_im = np.array(imageio.imread(prefix + 'sat.png'))
...
def load_rect(region, rect, load_func=load_tile, mode='all'):
            ...
            for k, im in tile_ims.iteritems():
                ...
                if k not in full_ims:
                    full_ims[k] = numpy.zeros(
                        (full_rect.lengths().x // scale,
                         full_rect.lengths().y // scale, im.shape[2]),
                        dtype='uint8')
    ...
    for k in full_ims:
        ...
        full_ims[k] = full_ims[k][crop_rect.start.x // scale:crop_rect.end.x //
                                  scale, crop_rect.start.y //
                                  scale:crop_rect.end.y // scale, :]
    return full_ims
...
class TileCache(object):
    ...
    def get(self, region, rect):
        k = '{}.{}.{}.{}.{}'.format(region, rect.start.x, rect.start.y,
...
class Tiles(object):
    def __init__(self, paths_per_tile_axis, segment_length, parallel_tiles,
                 tile_mode):
    ...
    def prepare_training(self):
        ...
        self.all_starting_locations = get_starting_locations(
            self.gcs, self.segment_length)
        ...
        self.train_tiles = [
            tile for tile in self.train_tiles
            if tile.region in TRAINING_REGIONS
        ]
        print('go from {} to {} tiles after excluding regions'.format(
            old_len, len(self.train_tiles)))
    ...
    def get_tile_data(self, region, rect):
        ...
        starting_locations = get_starting_locations(self.gcs,
                                                    self.segment_length,
                                                    region=region)[k]
        starting_locations = [
            loc for loc in starting_locations
